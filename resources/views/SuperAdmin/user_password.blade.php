...
<div class="container-fluid mt-3">
    ...
    <div class="card">
        <div class="card-header bg-white d-flex align-items-center justify-content-between">
            <h5 class="mb-0">User Password (Send Credentials)</h5>
            <div>
        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-lg-4 mb-3">
                    <label class="font-weight-bold">SMS Message Template</label>
                    <textarea id="smsTemplate" class="form-control" rows="3">HABARI, Account yako ipo tayari. LOGIN KWA username:{username} password:{password}</textarea>
                    <small class="text-muted">Placeholders: {username}, {password}, {school}</small>
                </div>
            </div>
...
$(document).ready(function() {
    ...
    async function loadUsers() {
        ...
        try {
            const res = await $.ajax({
                url: '{{ route("superadmin.user_password.users") }}',
                type: 'GET',
    ...
    $('#btnSendSms').on('click', async function() {
        ...
        const template = $('#smsTemplate').val();
        if (!template || template.trim().length === 0) {
            Swal.fire({ icon: 'warning', title: 'Message required', text: 'Please enter SMS message template.' });
            return;
        }

        const confirmed = await Swal.fire({
            title: 'Send SMS?',
            text: `You are about to send credentials SMS to ${selectedRows.length} users.`,
            icon: 'question',
        ...
        $('#schoolID, #userType, #smsTemplate, #selectAllUsers').prop('disabled', true);
        ...
        for (let i = 0; i < total; i++) {
            ...
            try {
                const response = await $.ajax({
                    url: '{{ route("superadmin.user_password.send_sms") }}',
                    type: 'POST',
                    data: payload
                });
                ...
                if (response && response.success) {
                    ...
                    const msg = (response && (response.error || response.message)) || 'Failed';
                    if (!firstErrorMessage) firstErrorMessage = msg;
                    statusCol.html('<i class="bi bi-exclamation-circle-fill text-danger" title="'+ safeText(msg) +'"></i> <span class="small text-danger">'+ safeText(msg) +'</span>');
                    failed++;
                }
            } catch (err) {
        ...
        $('#schoolID, #userType, #smsTemplate, #selectAllUsers').prop('disabled', false);
        $('#selectAllUsers').prop('checked', false);

        // Re-enable checkboxes (keep rows with no phone disabled)
